...
class ViewController: UIViewController, NarrativeViewDelegate {
    ...
    func isNoun(s: String) -> Bool {
        ...
        let options: NSLinguisticTaggerOptions = [NSLinguisticTaggerOptions.OmitWhitespace, NSLinguisticTaggerOptions.OmitOther, NSLinguisticTaggerOptions.OmitPunctuation, NSLinguisticTaggerOptions.JoinNames]
        tagger.string = sentence
        var ret = false
        tagger.enumerateTagsInRange(NSMakeRange(0, sentence.characters.count), scheme: NSLinguisticTagSchemeLexicalClass, options: options) { (tag:String, tokenRange:NSRange, _, _) in
    ...
    func isIngVerb(s: String) -> Bool {
        if let _ = s.rangeOfString("ing$", options: .RegularExpressionSearch) {
            return true
        }
        return false
    }
