...
public protocol NarrativeViewDelegate {
    ...
    - parameter wereValid: is true if all fields were valid
    */
...
public class NarrativeView: UIScrollView, UITextFieldDelegate {
    ...
    */
    private struct Layout {
        ...
        func layoutItem(forView forView: UIView) -> NarrativeItem? {
            for item in items {
                switch item {
        ...
        func nextLayoutItem(forView forView: UIView) -> NarrativeItem? {
            var foundItem = false
            for item in items {
                switch item {
                case let .Field(textfield, _):
    ...
    - parameter createTextField:  closure to create the text field
    - parameter valueChanged: Closure called when value changes, should return or not the field is consider valid
    
    - returns: The class instance
    */
    public func textField(createTextField createTextField: () -> UITextField, valueChanged: UITextField -> Bool) -> Self {
    ...
    - parameter createLabel: closure to create a UILabel
    - parameter text: The label text
    
    - returns: The class instance
    */
    public func label(createLabel createLabel: Void -> UILabel, text: String) -> Self {
        let words = text.characters.split { $0 == " " }.map { String($0) }
    ...
    - parameter makeButton: closure to make a button
    ...
    */
    ...
    private class AccessibleLabelSpan {
        ...
        func extend(frame frame: CGRect, text: String?) -> Bool {
    ...
    - parameter containerView: The view containing the flow layout
    
    - returns: An array of accesibility items
    */
    private func accessibilityItems(containerView containerView: UIView) -> [AnyObject] {
        ...
        for item in layout.items {
            switch item {
            case let .Field(textField, _):
                let _ = accessibleLabelSpan.map(appendItem)
    ...
    override public func accessibilityElementCount() -> Int {
        guard let elements = accessibilityElements else { return 0 }
        return elements.count
    }
    override public func accessibilityElementAtIndex(index: Int) -> AnyObject? {
        guard let elements = accessibilityElements else { return nil }
    ...
    public func textFieldShouldBeginEditing(textField: UITextField) -> Bool {
        if let _ = layout.nextLayoutItem(forView: textField) {
