...
struct NarrativeLayout {
    ...
    :param: layoutItems The items to be laid out
    ...
    */
    func layout(layoutItems: [NarrativeItem]) -> [UIView] {
        func resizeTextField(textField: UITextField) {
            ...
            var newWidth = (widthThatFits > rowWidth ? rowWidth : widthThatFits)
        ...
        for item in layoutItems {
            ...
            case let .Field(textField, _):
                ...
                (row, xOffset) = positionView(textField, row, xOffset, laidOut.last)
                laidOut.append(textField)
            case .Label(let label):
                resizeLabel(label)
                (row, xOffset) = positionView(label, row, xOffset, laidOut.last)
                laidOut.append(label)
                label.isAccessibilityElement = false
            case let .Button(button, _):
                resizeButton(button)
                (row, xOffset) = positionView(button, row, xOffset, laidOut.last)
    ...
    //MARK: - Private
    private func calculateSpaceRemaining(#rowWidth: CGFloat, currentOffset: CGFloat) -> CGFloat {
        let space = rowWidth - currentOffset
        return space > 0 ? space : 0
    }
    private func leadSpacing(#view: UIView, currentOffset: CGFloat, previousView: UIView?) -> CGFloat{
        if previousView == nil {
            return 0
